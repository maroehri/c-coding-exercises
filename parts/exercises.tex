...
\mintinline{c}{findMin()} gibt, die als Argumente ein Array aus Zahlen und die
...
\TopicLevel{Arrays}{2}\vspace{10pt}

\begin{enumerate}

\item Gegeben ist ein \mintinline{c}{int}-Array mit beliebigen Ganzzahlwerten.

Schreiben Sie eine Funktion \mintinline{c}{countNumbers()}, die ermittelt, wie
oft jeder der Werte 0 bis 9 in diesem Array vorkommt. Das Ergebnis soll in einem
separaten Array der Länge 10 gespeichert werden.

Parameter:
\begin{itemize}
    \item \mintinline{c}{data} -- Eingabe-Array mit Ganzzahlwerten
    \item \mintinline{c}{size} -- Anzahl der Elemente in \mintinline{c}{data}
    \item \mintinline{c}{count} -- Ausgabe-Array der Länge 10, in das die
    Häufigkeiten geschrieben werden sollen
\end{itemize}

\begin{myboxtext}[title=Beispiel]{example}
Für das Array
\texttt{[10, \highlight{2}, \highlight{2}, 22, \highlight{3}, \highlight{2}, -3, \highlight{6}, \highlight{3}, 17]}

soll das Ergebnis ein Array mit folgendem Inhalt sein:

\texttt{[0, 0, 3, 2, 0, 0, 1, 0, 0, 0]}
\end{myboxtext}

\item Schreiben Sie eine Funktion \mintinline{c}{printHistogram()}, die das
Häufigkeits-Array als horizontales Balkendiagramm auf der Konsole ausgibt. Jeder
Balken besteht aus *-Zeichen. Werte mit Häufigkeit 0 werden mit einer leeren
Zeile dargestellt.

\begin{myboxtext}[title=Beispiel]{example}
Für das Ergebnis \texttt{[0, 0, 3, 2, 0, 0, 1, 0, 0, 0]}:
...
Gegeben ist ein String, der nur aus Kleinbuchstaben (\texttt{a}-\texttt{z})
besteht. Schreiben Sie eine Funktion \mintinline{c}{compress()}, die einen
String mit einer einfachen Lauflängenkodierung kodiert. Dabei wird jede Folge
gleicher aufeinanderfolgender Zeichen durch das Zeichen selbst und die Anzahl
seines Auftretens ersetzt.

Parameter:
\begin{itemize}
    \item \mintinline{c}{input} -- Eingabe-String (nur Kleinbuchstaben)
    \item \mintinline{c}{output} -- Ausgabe-String, in den das kodierte
    Ergebnis geschrieben wird (ausreichend groß)
\end{itemize}

Auch einzeln vorkommende Zeichen erhalten eine Zahl (z.\,B.
"\texttt{c}" → "\texttt{c1}"). Sie können davon ausgehen, dass kein Zeichen mehr
als 9-mal hintereinander vorkommt.

\textit{Hinweis:} Die Anzahl muss als Zeichen in den Ausgabe-String geschrieben
werden. Die Zahl \texttt{3} wird z.\,B. durch den Ausdruck \texttt{'0' + 3} zum
Zeichen \texttt{'3'}.

\begin{myboxtext}[title=Beispiel]{example}
\begin{tblr}{
    colspec = {Q[c,m,2cm] X[c,m] X[c,m] X[c,m]},
    row{1}={font=\bfseries\sffamily},
    row{even}={bg=mittelgrau},
}
\toprule
Eingabe             & Ausgabe           \\ \midrule
"\texttt{aaabbc}"   & "\texttt{a3b2c1}" \\
"\texttt{aaa}"      & "\texttt{a3}"     \\
"\texttt{abc}"      & "\texttt{a1b1c1}" \\
"\texttt{}"         & "\texttt{}"       \\
\bottomrule
\end{tblr}
\end{myboxtext}
...
Erstellen Sie eine Funktion \mintinline{c}{findMaxChar()}, die basierend auf
einem an die Funktion übergebenen String das Zeichen ermittelt, das am
häufigsten in diesem String vorkommt. Die Funktion soll das gefundene Zeichen
zurückgeben und die zugehörige Anzahl über einen Pointer-Parameter an den
Aufrufer übergeben.
...
Schreiben Sie eine Funktion \mintinline{c}{createVowelFreeString()}, die alle
...
\TopicLevel{Arrays, Dynamische Speicherverwaltung}{2}\vspace{10pt}

Schreiben Sie eine Funktion \mintinline{c}{clampAll()}, die aus einem
Eingabearray ein neues Array erzeugt, in dem alle Werte größer als ein
Maximalwert auf diesen Maximalwert begrenzt werden. Werte, die bereits kleiner
oder gleich dem Maximalwert sind, werden unverändert übernommen.

Das Ergebnisarray soll dynamisch auf dem Heap allokiert und über einen
Doppel-Pointer-Parameter an den Aufrufer zurückgegeben werden. Die Funktion soll
\mintinline{text}{0} bei Erfolg und \mintinline{text}{-1} im Fehlerfall
zurückgeben.

\begin{myboxtext}[title=Beispiel]{example}
Eingabe: \mintinline{text}{[3, 12, 7, 20, 5]}, Maximalwert \mintinline{text}{10}\\
Ergebnis: \mintinline{text}{[3, 10, 7, 10, 5]}
\end{myboxtext}
...
int main(void) {
    ...
    printf("Eingabe:   ");
    for (int i = 0; i < n; i++) printf("%d ", source[i]);
    printf("\nErgebnis:  ");
...
Erstellen Sie eine Funktion \mintinline{c}{findHighAndLow()}, die die höchsten
...
\textit{Hinweis:} Sie können die Funktion \mintinline{c}{sprintf()} verwenden,
...
Erstellen Sie eine Funktion \mintinline{c}{isValidIpv4()}, die überprüft, ob ein
...
\mintinline{c}{true} zurückgeben, wenn die Eingabe eine gültige IPv4-Adresse
ist, und andernfalls \mintinline{c}{false}.
...
\TopicLevel{Strukturen, Dynamische Speicherverwaltung}{4}\vspace{10pt}

Ein \textit{Stapel} (engl. \textit{Stack}) ist eine Datenstruktur, die nach dem
LIFO-Prinzip (\textit{Last In, First Out}) arbeitet: Werte werden oben auf den
Stapel gelegt und auch wieder von oben heruntergenommen. In dieser Aufgabe
sollen Sie einen Stapel mit dynamisch wachsender Kapazität in C implementieren.
...
Das Feld \mintinline{c}{data} zeigt auf einen Buffer der Größe
\mintinline{c}{capacity}. Davon sind die ersten \mintinline{c}{size} Elemente
tatsächlich genutzt; das oberste Stapelelement steht an Index
\mintinline{c}{size - 1}.

\begin{enumerate}

\item Erstellen Sie eine Funktion \mintinline{c}{stackInit()}, die einen Stapel
initialisiert. Die Funktion bekommt einen Pointer auf einen
\mintinline{c}{intStack} sowie eine Anfangskapazität übergeben. Allokieren Sie
einen Buffer der angegebenen Größe auf dem Heap und setzen Sie die Felder der
Struktur entsprechend. Im Fehlerfall soll \mintinline{c}{-1} zurückgegeben
werden, sonst \mintinline{c}{0}.

\item Erstellen Sie eine Funktion \mintinline{c}{stackPush()}, die einen Wert
oben auf den Stapel legt. Falls der Buffer dafür nicht mehr ausreicht, soll die
Kapazität \textit{verdoppelt} werden, bevor der Wert hinzugefügt wird. Im
Fehlerfall soll \mintinline{c}{-1} zurückgegeben werden, sonst
\mintinline{c}{0}.

\item Erstellen Sie eine Funktion \mintinline{c}{stackPop()}, die das oberste
Element vom Stapel entfernt und über einen Pointer-Parameter zurückgibt. Ist der
Stapel leer, soll \mintinline{c}{-1} zurückgegeben werden, sonst
\mintinline{c}{0}. Die Kapazität des Buffers muss nicht angepasst werden.

\item Erstellen Sie eine Funktion \mintinline{c}{stackFree()}, die den vom
Stapel belegten Speicher wieder freigibt und die Felder der Struktur
zurücksetzt.
...
\begin{mybox}[Bildschirmausgabe]{console}
Stapel hat 4 Elemente, Kapazität: 4
Pop: 40
Pop: 30
Pop: 20
Pop: 10
...
\TopicLevel{Pointer, Dyn. Speicherverwaltung, Sortieren}{4}\vspace{10pt}
