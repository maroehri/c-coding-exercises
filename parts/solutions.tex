...
Tokens zu zerlegen, die durch Leerzeichen getrennt sind. Da
\mintinline{c}{strtok()} den ursprünglichen String modifiziert, wird zuerst eine
Kopie des Eingabestrings erstellt, um die Originaldaten nicht zu verändern.
Anschließend wird jedes Token mithilfe von \mintinline{c}{strtol()} in eine
Ganzzahl umgewandelt, wobei der erste Wert als Initialisierung für das Minimum
und Maximum dient. In einer Schleife werden alle weiteren Tokens verarbeitet, um
das Minimum und Maximum zu bestimmen. Das Ergebnis wird schließlich mithilfe der
Funktion \mintinline{c}{sprintf()} in das Format \mintinline{c}{"max min"}
gebracht und in den Ergebnisstring geschrieben.
