...
\begin{codebox}[lang=c, fontsize=\small]
#include <stdio.h>
...
\chapter{Häufigkeiten der Werte 0–9 zählen und als Histogramm ausgeben}

\section*{Funktion \texttt{countNumbers()}}

Die Funktion \mintinline{c}{countNumbers()} ermittelt, wie oft jeder der Werte
0 bis 9 in einem gegebenen Array vorkommt. Hierfür nutzen wir einen typischen
Index-Mapping-Ansatz: das Ergebnis-Array \mintinline{c}{count} hat genau 10
Einträge, wobei der Index dem zu zählenden Wert entspricht. Der Eintrag
\mintinline{c}{count[3]} enthält also am Ende die Anzahl der 3en im
Eingabe-Array.

Zunächst muss das Ergebnis-Array mit Nullen initialisiert werden, da wir
ansonsten mit undefinierten Werten arbeiten würden. Anschließend durchlaufen
wir das Eingabe-Array und erhöhen für jeden gefundenen Wert im Bereich 0--9
den entsprechenden Eintrag im Ergebnis-Array. Werte außerhalb dieses Bereichs
(z.\,B. negative Zahlen oder Zahlen größer 9) werden ignoriert.

\begin{codebox}[lang=c, fontsize=\small]
void countNumbers(const int *data, int size, int *count) {
    // Ergebnis-Array mit Nullen initialisieren
    for (int i = 0; i < 10; i++) {
        count[i] = 0;
    }
...
\section*{Funktion \texttt{printHistogram()}}

Die Funktion \mintinline{c}{printHistogram()} gibt das Häufigkeits-Array als
horizontales Balkendiagramm auf der Konsole aus. Hierfür nutzen wir zwei
verschachtelte Schleifen: die äußere Schleife durchläuft die Werte 0 bis 9,
die innere Schleife gibt für jeden Wert genau so viele *-Zeichen aus, wie der
entsprechende Eintrag im Häufigkeits-Array angibt.

Werte mit Häufigkeit 0 werden automatisch korrekt behandelt -- die innere
Schleife wird in diesem Fall einfach gar nicht ausgeführt, sodass nur die
Zeilenbeschriftung mit anschließendem Zeilenumbruch ausgegeben wird.
...
\chapter{Lauflängenkodierung eines Strings}

Bei der Lauflängenkodierung wird jede Folge gleicher aufeinanderfolgender
Zeichen durch das Zeichen selbst und die Anzahl seines Auftretens ersetzt.
Wir benötigen daher zwei Indizes: \mintinline{c}{readPos} zum Lesen aus dem
Eingabe-String und \mintinline{c}{writePos} zum Schreiben in den
Ausgabe-String. Beide Indizes laufen unabhängig voneinander, da der Ausgabe-
String je nach Eingabe kürzer oder länger sein kann als der Eingabe-String.

Die äußere \mintinline{c}{while}-Schleife läuft so lange, bis das Ende des
Eingabe-Strings (\mintinline{c}{'\0'}) erreicht ist. In jedem Durchlauf der
äußeren Schleife wird das aktuelle Zeichen in \mintinline{c}{currentChar}
gespeichert und der Zähler \mintinline{c}{count} mit 1 initialisiert.
Anschließend wird der \mintinline{c}{readPos}-Index erhöht, da das aktuelle
Zeichen ja bereits einmal gezählt wurde.

In der inneren \mintinline{c}{while}-Schleife prüfen wir, ob das nächste
Zeichen im Eingabe-String dem aktuellen Zeichen entspricht. Solange dies der
Fall ist, erhöhen wir \mintinline{c}{count} und \mintinline{c}{readPos}.
Endet die innere Schleife, sind wir am Ende einer Zeichenfolge angekommen
oder am Ende des Eingabe-Strings.

Anschließend schreiben wir das Zeichen und die Anzahl in den Ausgabe-String.
Da \mintinline{c}{count} eine Ganzzahl ist und im Ausgabe-String ein Zeichen
benötigt wird, müssen wir die Zahl in das entsprechende Zeichen umwandeln.
Das geht elegant über den Ausdruck \mintinline{c}{'0' + count}: addiert man
die ASCII-Werte des Zeichens \mintinline{c}{'0'} (= 48) und der Zahl
\mintinline{c}{count}, erhält man den ASCII-Wert des entsprechenden
Zifferzeichens.

Zum Schluss muss der Ausgabe-String mit \mintinline{c}{'\0'} terminiert
werden, damit er als gültiger String behandelt werden kann.
...
Diese Aufgabe lässt sich auf zwei grundlegend verschiedene Arten lösen. Beide
Ansätze sind algorithmisch korrekt, unterscheiden sich aber deutlich in
Effizienz und Speicherverbrauch.

\section*{Ansatz 1: Verschachtelte Schleifen}

Der intuitive Ansatz besteht darin, den String mehrfach zu durchlaufen. In einer
äußeren Schleife wird jeweils ein Zeichen aus dem String entnommen und in der
Variablen \mintinline{c}{currentChar} gespeichert. Anschließend zählen wir in
einer inneren Schleife die Häufigkeit dieses Zeichens im gesamten String, wofür
die Variable \mintinline{c}{currentOccurrence} verwendet wird. Die innere
Schleife muss dabei nur ab der aktuellen Position der äußeren Schleife laufen,
da Zeichen vor dieser Position bereits in einem früheren Durchlauf der äußeren
Schleife betrachtet wurden.

Falls das aktuell betrachtete Zeichen identisch mit dem bis dahin am häufigsten
gefundenen Zeichen (\mintinline{c}{maxChar}) ist, können wir dieses Zeichen
überspringen, um redundante Zählungen zu vermeiden.
...
Häufigkeit ist. Ist dies der Fall, aktualisieren wir \mintinline{c}{maxChar} und
\mintinline{c}{maxOccurrence} entsprechend.

Am Ende wird die maximale Häufigkeit über den Pointer-Parameter
\mintinline{c}{occurrence} an den Aufrufer übergeben, und das Zeichen wird als
...
\section*{Ansatz 2: Häufigkeitsarray}

Ein deutlich effizienterer Ansatz nutzt ein \textit{Häufigkeitsarray}: ein Array
von ganzen Zahlen, in dem an jedem Index gespeichert wird, wie oft das Zeichen
mit diesem Wert im String vorkommt. In C werden \mintinline{c}{char}-Werte
intern als Ganzzahlen gemäß ASCII-Tabelle gespeichert. Das Zeichen
\mintinline{c}{'a'} hat z.\,B. den Wert \texttt{97} und kann daher direkt als
Index verwendet werden: \mintinline{c}{frequency['a']} ist gleichbedeutend mit
\mintinline{c}{frequency[97]}.

Die Lösung läuft in zwei Phasen ab. In der ersten Phase wird der String einmal
durchlaufen und für jedes Zeichen der entsprechende Eintrag im Häufigkeitsarray
erhöht. In der zweiten Phase wird das Häufigkeitsarray einmal durchlaufen, um
den Index mit dem höchsten Wert zu finden -- dieser Index entspricht dem
gesuchten Zeichen.
...
Der erste Ansatz benötigt nur wenige Variablen als Hilfsspeicher, ist dafür aber
mit zwei verschachtelten Schleifen relativ langsam: bei einem String der Länge
$n$ werden bis zu $n^2$ Vergleiche durchgeführt. Der zweite Ansatz benötigt
zusätzlichen Speicher für das Häufigkeitsarray (256 \mintinline{c}{int}-Werte),
kommt aber mit zwei einfachen Schleifen aus und ist dadurch deutlich schneller.
Bei einem String mit 10\,000 Zeichen sind das ca. 100\,000\,000 Operationen im
ersten Ansatz gegenüber nur ca. 10\,256 Operationen im zweiten -- ein
Unterschied um Faktor 10\,000.

In der Praxis ist der zweite Ansatz fast immer vorzuziehen. Der erste Ansatz hat
aber den Vorteil, dass er ohne Annahmen über den Wertebereich der Zeichen
funktioniert. Der Häufigkeitsarray-Ansatz setzt voraus, dass alle Zeichen im
Bereich \mintinline{c}{0}--\mintinline{c}{255} liegen -- was bei reinen
ASCII-Strings immer der Fall ist, bei Multibyte-Zeichen (z.\,B. UTF-8 mit
deutschen Umlauten) aber zu falschen Ergebnissen führen kann.
...
\chapter{Werte eines Arrays auf einen Maximalwert begrenzen}

Die Funktion \mintinline{c}{clampAll()} erstellt ein neues Array auf dem Heap,
das die begrenzten Werte des Eingabe-Arrays enthält. Da die Funktion einen
Pointer auf das neu allokierte Array an den Aufrufer übergeben muss, wird der
\textit{Doppel-Pointer} \mintinline{c}{int **target} als Parameter verwendet.
Über diesen können wir den Pointer im Aufrufer auf den allokierten Speicher
zeigen lassen.

Im Funktionsrumpf allokieren wir zunächst Speicher für \mintinline{c}{n}
\mintinline{c}{int}-Werte mit \mintinline{c}{malloc()}. Schlägt die Allokation
fehl (\mintinline{c}{malloc()} liefert \mintinline{c}{NULL}), gibt die
Funktion \mintinline{c}{-1} zurück, ohne weitere Operationen durchzuführen.

Anschließend durchlaufen wir das Eingabe-Array mit einer for-Schleife. Für
jeden Wert wird geprüft, ob er größer als der Maximalwert
\mintinline{c}{max} ist. Falls ja, wird der Wert auf \mintinline{c}{max}
begrenzt; andernfalls wird er unverändert übernommen.

Damit die Hilfsvariable \mintinline{c}{result} für den Aufrufer sichtbar
wird, müssen wir sie am Ende dem Doppel-Pointer-Parameter zuweisen. Mit
\mintinline{c}{*target = result} schreiben wir den Pointer auf den
allokierten Speicher in die Variable, auf die \mintinline{c}{target} zeigt --
also in den Pointer im Aufrufer.

Wichtig ist hier, eine \textit{Hilfsvariable} \mintinline{c}{result} für den
allokierten Speicher zu verwenden, anstatt direkt mit \mintinline{c}{*target}
zu arbeiten. So bleibt der Code übersichtlich und die Doppel-Pointer-Logik
nur am Ende einmal sichtbar.

Der Aufrufer ist nach Abschluss der Funktion dafür verantwortlich, den
allokierten Speicher mittels \mintinline{c}{free()} wieder freizugeben.

\begin{codebox}[lang=c, fontsize=\small]
int clampAll(const int source[], const int n, const int max,
             int **target) {
...
Die erste Lösung verwendet die Funktion \mintinline{c}{strtol()}, um die Zahlen
...
Ganzzahl umgewandelt. Die Funktion \mintinline{c}{strtol()} verarbeitet zunächst
...
Argument (\mintinline{c}{&parsePtr}) übergeben wird, auf das nächste
nicht-parsbare Zeichen gesetzt. Auf diese Weise kann der String schrittweise
verarbeitet werden, ohne ihn zu verändern.

Das Ergebnis wird am Ende mithilfe der Funktion \mintinline{c}{sprintf()} in
das Format \mintinline{c}{"max min"} gebracht und in den Ergebnisstring
geschrieben.
...
Die zweite Lösung nutzt \mintinline{c}{strtok()}, um die Eingabezeichenkette in
Tokens zu zerlegen, die durch Leerzeichen getrennt sind. Da die verwendete
Funktion \mintinline{c}{strtok()} den zu verarbeitenden String modifiziert, wird
...
\mintinline{c}{strtok()} in einzelne, durch Leerzeichen getrennte Tokens
zerlegt.

Das erste Token wird mit \mintinline{c}{strtol()} in eine Ganzzahl umgewandelt
...
\mintinline{c}{sprintf()} im Format \mintinline{c}{"max min"} in den
...
\begin{codebox}[lang=c, fontsize=\small, linenos=true]
#include <stdio.h>
#include <string.h> // für strlen()
...
Die Funktion \mintinline{c}{isValidIpv4()} prüft, ob ein gegebener String eine
...
Da die zur Zerlegung verwendete Funktion \mintinline{c}{strtok()} den
...
Validierung unterzogen werden: Zunächst wird mit \mintinline{c}{isdigit()}
...
sind. Schließlich wird das Oktett mit \mintinline{c}{strtol()} in eine Zahl
...
wurden. Die Funktion gibt \mintinline{c}{false} zurück, wenn die Eingabe leer
oder \mintinline{c}{NULL} ist, ein Oktett ungültige Zeichen oder unerlaubte
...
\mintinline{c}{true} zurückgegeben.
...
bool isValidIpv4(const char *ipAddress) {
    ...
    // Die min. Länge besteht aus  4 Ziffern und 3 Punkten (= 7)
    // Die max. Länge besteht aus 12 Ziffern und 3 Punkten (= 15)
    ...
    while (token != NULL) {
        // Ein Oktet mit mehr als einer Ziffer darf nicht
...
\chapter{Stapel mit dynamisch wachsender Kapazität}

\section*{Funktion \texttt{stackInit()}}

Die Funktion \mintinline{c}{stackInit()} initialisiert einen Stapel mit einer
gegebenen Anfangskapazität. Hierfür wird mit \mintinline{c}{malloc()}
Speicher für \mintinline{c}{initialCapacity} \mintinline{c}{int}-Werte auf
dem Heap reserviert. Schlägt die Allokation fehl, wird \mintinline{c}{-1}
zurückgegeben, ohne weitere Felder der Struktur zu setzen.

Im Erfolgsfall werden die Strukturfelder \mintinline{c}{size} (auf 0) und
\mintinline{c}{capacity} (auf \mintinline{c}{initialCapacity}) initialisiert.
Der Stapel ist jetzt einsatzbereit, aber noch leer.
...
\section*{Funktion \texttt{stackPush()}}

Die Funktion \mintinline{c}{stackPush()} legt einen Wert oben auf den Stapel.
Zunächst wird geprüft, ob der Buffer noch ausreichend groß ist
(\mintinline{c}{size < capacity}). Ist dies nicht der Fall, muss die
Kapazität verdoppelt werden, bevor der Wert angefügt werden kann.

Für die Vergrößerung des Buffers verwenden wir die
\mintinline{c}{realloc()}-Funktion. Wichtig ist dabei, das Ergebnis von
\mintinline{c}{realloc()} \textit{nicht} direkt
\mintinline{c}{stack->data} zuzuweisen: schlägt \mintinline{c}{realloc()}
fehl, würde der ursprüngliche Pointer dabei verloren gehen, was zu einem
Memory Leak führen würde. Stattdessen nutzen wir eine Hilfsvariable
\mintinline{c}{newData}, die wir erst nach erfolgreicher Reallokation dem
\mintinline{c}{stack->data}-Feld zuweisen.

Nach der erfolgreichen Vergrößerung (oder wenn keine Vergrößerung nötig war)
wird der neue Wert an die nächste freie Position
(\mintinline{c}{stack->data[stack->size]}) geschrieben und
\mintinline{c}{size} um eins erhöht.
...
\section*{Funktion \texttt{stackPop()}}

Die Funktion \mintinline{c}{stackPop()} entfernt das oberste Element vom
Stapel und gibt dessen Wert über den Pointer-Parameter
\mintinline{c}{value} zurück. Zunächst wird geprüft, ob der Stapel überhaupt
Elemente enthält. Ist er leer, wird \mintinline{c}{-1} zurückgegeben, ohne
\mintinline{c}{value} zu verändern.

Andernfalls wird zuerst \mintinline{c}{size} dekrementiert und anschließend
das Element an dieser neuen Position gelesen. Diese Reihenfolge ist wichtig:
das oberste Element steht an Index \mintinline{c}{size - 1}, also genau dort,
wo \mintinline{c}{size} nach der Dekrementierung hinzeigt. Würden wir die
Reihenfolge umdrehen, müssten wir mit dem Index \mintinline{c}{size}
arbeiten, was leicht zu Verwirrung führen kann.

Die Kapazität des Buffers muss nicht angepasst werden -- der freigewordene
Speicher kann in einem späteren \mintinline{c}{stackPush()}-Aufruf
...
\section*{Funktion \texttt{stackFree()}}

Die Funktion \mintinline{c}{stackFree()} gibt den Speicher des Buffers wieder
frei und setzt die Strukturfelder auf ihre Initialwerte zurück. Das Setzen
von \mintinline{c}{stack->data} auf \mintinline{c}{NULL} ist wichtig, um
sogenannte \textit{Dangling Pointer} zu vermeiden -- also Pointer, die auf
bereits freigegebenen Speicher zeigen und bei einem versehentlichen Zugriff
zu undefinierten Verhalten führen würden.
